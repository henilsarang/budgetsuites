...
CREATE UNIQUE INDEX `email` ON `budgetsuites`.`user` (`email` ASC) VISIBLE;
...
-- -----------------------------------------------------
-- Table `budgetsuites`.`transaction`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `budgetsuites`.`transaction` ;

CREATE TABLE IF NOT EXISTS `budgetsuites`.`transaction` (
...
-- -----------------------------------------------------
-- Table `budgetsuites`.`transaction_room`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `budgetsuites`.`transaction_room` ;

CREATE TABLE IF NOT EXISTS `budgetsuites`.`transaction_room` (
...
-- -----------------------------------------------------
-- Placeholder table for view `budgetsuites`.`booking`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `budgetsuites`.`booking` (`transaction_id` INT, `transaction_room_id` INT, `user_id` INT, `guest_id` INT, `total_price` INT, `cancellation_charge` INT, `date_in` INT, `date_out` INT, `status` INT, `amount_paid` INT, `stripe_id` INT, `room_id` INT, `room_price` INT);

-- -----------------------------------------------------
-- View `budgetsuites`.`booking`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `budgetsuites`.`booking`;
DROP VIEW IF EXISTS `budgetsuites`.`booking` ;
USE `budgetsuites`;
...
USE `budgetsuites`$$
DROP TRIGGER IF EXISTS `budgetsuites`.`hotel_BEFORE_INSERT` $$
USE `budgetsuites`$$
CREATE DEFINER = CURRENT_USER TRIGGER `budgetsuites`.`hotel_BEFORE_INSERT` BEFORE INSERT ON `hotel` FOR EACH ROW
BEGIN
...
USE `budgetsuites`$$
DROP TRIGGER IF EXISTS `budgetsuites`.`hotel_BEFORE_UPDATE` $$
USE `budgetsuites`$$
CREATE DEFINER = CURRENT_USER TRIGGER `budgetsuites`.`hotel_BEFORE_UPDATE` BEFORE UPDATE ON `hotel` FOR EACH ROW
BEGIN
SET     NEW.geo_point = ST_GeomFromText(CONCAT('POINT(',NEW.latitude,' ',NEW.longitude,')'),4326);
END$$
